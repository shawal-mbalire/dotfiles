...
Singleton {
    ...
    function setVolume(node, value) {
        if (node && node.audio) node.audio.volume = Formatters.clamp01(value);
    }

    function bumpVolume(node, delta) {
        if (node && node.audio)
            node.audio.volume = Formatters.clamp01(node.audio.volume + delta);
    }

    // Filters live in EasyEffects (a PipeWire filter-chain front end); the
    // shell just launches it.
    function openTweaker() {
        Quickshell.execDetached([Config.audioTweaker]);
    }
}
