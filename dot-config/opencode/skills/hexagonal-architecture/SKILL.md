...
20. **Verifiable by Default** — Every behavior has a test at the lowest tier that can catch its failure; every failure path has a fault-injection test; nothing merges unless `just verify` is green.
21. **Reproducible Runs** — Inject `TimePort` and `RandomPort`; pin dependencies and toolchains. Any failure can be replayed deterministically from its captured input.
22. **Verify the Verifier** — Property tests cover all inputs; mutation testing proves the tests catch defects; the mutation score for `domain/` is a gate.
23. **Prove the Critical Core** — The small, high-risk algorithm/protocol gets a formal model (TLA+/Kani/CBMC/Dafny), not just tests.
24. **Fail Safe in Prod** — Ship behind `FeatureFlagPort`, canary the roll-out, and bound the blast radius with timeouts, circuit breakers, and automatic rollback.
