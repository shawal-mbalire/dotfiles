...
def test_grid_chips_share_a_standard_width():
    ...
    assert cell["min-width"] == "88px"
    assert chip["min-width"] == "88px"
...
def test_control_center_has_low_opacity_section_dividers():
    sheet = theme_wf.build(base_config())
    components = {component.name: component for component in sheet.components}
    divider = {item.prop: item.value for item in components["section-divider"].declarations}
    assert divider["border-top"] == "1px solid alpha(@surface2, 0.5)"
    first = {item.prop: item.value for item in components["section-divider-first"].declarations}
    assert first["border-top"] == "none"
